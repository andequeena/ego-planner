...
namespace ego_planner
{
    ...
    struct PlanParameters
    {
        ...
        double max_vel_, max_acc_, max_jerk_; // physical limits jerk 跃度 加速度变化速率
        double ctrl_pt_dist;                  // distance between adjacient B-spline control points B样条相邻控制点之间的距离
        double feasibility_tolerance_;        // permitted ratio of vel/acc exceeding limits 速度/加速度超过限制时允许的“超限比例”
        double planning_horizen_;             // 算法每次规划未来 该变量秒 的可行轨迹。
